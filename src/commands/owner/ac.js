...
import { ANIMAL_COMPANY_CHANNEL_ID, ANIMAL_COMPANY_GUILD_ID, checkAnimalCompanyUpdates, getAnimalCompanyTrackerStatus, recordAnimalCompanyDevBuild } from '../../services/animalCompanyUpdateService.js';
...
export default {
  data: new SlashCommandBuilder().setName('ac').setDescription('Animal Company update tracker').setDMPermission(false).setDefaultMemberPermissions(0)
    .addSubcommandGroup(group => group.setName('mods').setDescription('Manage Animal Company updates')
      ...
      .addSubcommand(sub => sub.setName('dev-build').setDescription('Record and publish a developer build')
        .addStringOption(option => option.setName('version').setDescription('Developer build version, for example 1.86.0.3325').setRequired(true).setMaxLength(40)))
  ...
  async execute(interaction, client) {
    ...
    if (subcommand === 'check') {
      ...
      return interaction.editReply(`Check complete. Live build: **${result.liveVersion}**. Published **${result.posted}** new update(s).`);
    }
    if (subcommand === 'dev-build') {
      const version = interaction.options.getString('version').trim();
      if (!/^\d+(?:\.\d+){2,3}$/.test(version)) return interaction.editReply('Use a valid build version such as `1.86.0.3325`.');
      const result = await recordAnimalCompanyDevBuild(client, version);
      if (!result.ok) return interaction.editReply(result.code === 'database_unavailable' ? 'PostgreSQL is unavailable.' : `I cannot access <#${ANIMAL_COMPANY_CHANNEL_ID}>.`);
      return interaction.editReply(result.duplicate ? `Developer build **${version}** was already recorded.` : `Developer build **${version}** was posted in <#${ANIMAL_COMPANY_CHANNEL_ID}>.`);
    }
